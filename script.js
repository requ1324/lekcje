function startGame(s) {
  ...
  function getTopScores() {
    ...
    let cookie = document.cookie
      .split("; ")
      .find((row) => row.startsWith("topScores="));
    if (cookie) {
      try {
        scores = JSON.parse(cookie.split("=")[1]);
      } catch (e) {
        scores = [];
      }
    }
  ...
  function setTopScores(scores) {
    document.cookie = `topScores=${JSON.stringify(
      scores
  ...
  function checkWin() {
    if (matchedPairs == 8) {
      ...
      let newScore = { nick: user, time: parseFloat(timeTaken) };
  ...
  function displayTopScores() {
    ...
    scores.forEach((score, index) => {
      ...
      li.textContent = `${index + 1}. ${score.nick} - ${score.time} sekund`;
